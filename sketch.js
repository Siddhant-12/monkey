...
function setup() {
  ...
  //making the gr not being visible
  gr.visible=false;
...
function draw() {
  ...
  if(gameState===PLAY)
  {
    ...
    //making sprites visible on canvas
    drawSprites();
    
    //adding score and food
  text("Survival Time: "+ score,100,150);
  text("Food Collected:"+points,400,150);
    
    backgroundSprite.velocityX=-4;

    //adjusting the background sprite
    if(backgroundSprite.x===100)
    {
        backgroundSprite.x=300;
    }
   
    //makj=ing the monkey jump through space key
    ...
    //colliding monkey with gr
    monkey.collide(gr);
    
   //to add score when frame count reaches 60
    score = score+ Math.round(getFrameRate()/60);
    
  //fruit group function
